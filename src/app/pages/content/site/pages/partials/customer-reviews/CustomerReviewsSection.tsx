...
const CustomerReviewsSection = ({pageId, sectionId}: CustomerReviewsProps) => {
    ...
    const handleStore = useCallback((values: SectionFormFields, fns: any) => {
        submitRequest(
            storeSectionDatum,
            [pageId, sectionId, {...values, type: SectionTypeEnum.CUSTOMER_REVIEWS}],
            () => {
                wickApp.setAlert({
                    message: new AlertMessageGenerator('customer reviews section settings', Actions.EDIT, WickToastType.SUCCESS).message,
    ...
    return (
        <>
            <KTCard className="mb-8">
                <KTCardHeader text="Customer Reviews Block"/>
