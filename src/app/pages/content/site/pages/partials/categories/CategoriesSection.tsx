...
const CategoriesSection = ({pageId, sectionId}: CategoriesProps) => {
    ...
    const handleStore = useCallback((values: SectionFormFields, fns: any) => {
        submitRequest(
            storeSectionDatum,
            [pageId, sectionId, {...values, type: SectionTypeEnum.CATEGORIES}],
            () => {
                wickApp.setAlert({
                    message: new AlertMessageGenerator('categories section settings', Actions.EDIT, WickToastType.SUCCESS).message,
    ...
    return (
        <>
            <KTCard className="mb-8">
                <KTCardHeader text="Categories Block"/>
