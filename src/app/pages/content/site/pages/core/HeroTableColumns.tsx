...
import HeroSection from "../partials/hero/HeroSection.tsx";
import {BadgeCell} from "../../../../../modules/table/columns/BadgeCell.tsx";
import {capitalizeFirstLetter} from "../../../../../helpers/stringGenerator.ts";

const HeroTableColumns: ReadonlyArray<Column<any>> = [
    {
        Header: (props) => <CustomHeader tableProps={props} title='Main Image' className='p-4 min-w-125px'/>,
        id: 'main_image',
        Cell: ({...props}) => <>
            {props.data[props.row.index].main_image ? (
                <div className='symbol-label bg-secondary p-2'>
                    <img src={storageUrl(props.data[props.row.index].main_image[DEFAULT_RESPONSIVE_IMAGE_SIZE])}
    ...
    {
        Header: (props) => <CustomHeader tableProps={props} title='Main Image (Mobile Version)' className='p-4 min-w-125px'/>,
        id: 'main_image_mobile',
        Cell: ({...props}) => <>
            {props.data[props.row.index].main_image ? (
                <div className='symbol-label bg-secondary p-2'>
                    <img src={storageUrl(props.data[props.row.index].main_image['sm'])}
                         alt={props.data[props.row.index].title[DEFAULT_LANGUAGE]}
                         className='w-100'/>
                </div>
            ) : (
    ...
    {
        Header: (props) => <CustomHeader tableProps={props} title='Subtitle' className='min-w-125px'/>,
        id: 'subtitle',
        Cell: ({...props}) => <TextCellWithHelper text={props.data[props.row.index].subtitle[DEFAULT_LANGUAGE]}
                                                  helperText=""/>,
    },
    {
    ...
    {
        Header: (props) => <CustomHeader tableProps={props} title='Visibility' className='min-w-125px'/>,
        id: 'visibility',
        // Cell: ({...props}) => <TextCell text={props.data[props.row.index].hide}/>,
        Cell: ({...props}) => <BadgeCell status={props.data[props.row.index].hide ? 'Hidden' : 'Active'} color={props.data[props.row.index].hide  ? 'danger' : 'success'} align="left"/>,
    ...
    },
    {
        Header: (props) => (
            ...
            <CustomHeader tableProps={props} title='Actions' className='text-end min-w-100px p-4'/>
            // </Restricted>
        ),
        id: 'actions',
        Cell: ({...props}) => {
            const {page, section, handleRefresh} = usePageEdit()
            const modalId = `kt_hero_section_modal_${props.data[props.row.index].id}`
                            ...
                            <div className="modal-dialog">
                                <div className="modal-content">
                                    <div className="modal-header d-flex flex-row justify-content-between">
                                        <h5 className="modal-title">Edit Hero Slide</h5>
                                        <div
                                    ...
                                    <div className="modal-body">
                                        <HeroSection pageId={1} sectionId={2} onlyForm={true}
