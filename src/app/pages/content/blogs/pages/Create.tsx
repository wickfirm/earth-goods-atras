...
import {blogSchema, defaultFormFields, FormFields} from "../core/form.ts";
...
import {storeBlog} from "../../../../requests/content/Blog.ts";
...
const BlogCreate: React.FC = () => {
    ...
    useEffect(() => {
        wickApp.setPageTitle(generatePageTitle(Sections.CONTENT_BLOGS, PageTypes.CREATE))

        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    const handleCreate = (values: FormFields) => {
        submitRequest(storeBlog, [values], () => {
            // it"s product for sure
            wickApp.setAlert({
                message: new AlertMessageGenerator("blog", Actions.CREATE, WickToastType.SUCCESS).message,
    ...
    return (
        <>
            ...
            <Formik initialValues={defaultFormFields} validationSchema={blogSchema} onSubmit={handleCreate}
                    enableReinitialize>
                {
                    (formik) => (
                        <Form placeholder={undefined}
                            ...
                            <div className="d-flex flex-column gap-7 mb-7 me-lg-10"
                                ...
                                <BlogThumbnail formik={formik}/>
                                <BlogBanner formik={formik}/>
                            </div>
                            {/*Main column*/}
