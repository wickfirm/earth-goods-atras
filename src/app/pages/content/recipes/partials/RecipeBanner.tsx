...
const RecipeBanner: React.FC<Props> = ({form, setForm, formik}) => {
    ...
    useEffect(() => {
        ...
        if (form?.banner) {
            setInitialPreview(storageUrl(form?.banner[DEFAULT_RESPONSIVE_IMAGE_SIZE]));
        }
    ...
    return (
        <KTCard>
            <KTCardHeader text="Banner" className="border-0"/>
            <KTCardBody className="text-center pt-0">
                <div
                    className="image-input image-input-empty image-input-outline image-input-placeholder mb-3 bgi-position-center"
                    style={{backgroundImage: `url(${preview ?? initialPreview ?? '/media/svg/files/blank-image.svg'})`}}
                >
                    <div className="image-input-wrapper w-250px h-150px"/>
                    <label
                    ...
                    >
                        ...
                        <input
                            type="file"
                            accept=".png, .jpg, .jpeg"
                            onChange={(e) => handleFile(e, formik, 'banner')}
                        />
                ...
                <div className="text-muted fs-7">
                    Set the recipe banner. Only *.png, *.jpg, and *.jpeg image files are accepted.
                </div>
            </KTCardBody>
        </KTCard>
    );
